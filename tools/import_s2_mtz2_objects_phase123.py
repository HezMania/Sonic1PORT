#!/usr/bin/env python3
"""Phase 123: reconstruct retail Sonic 2 Metropolis Act 2 object sprite banks."""
...
def main():
    if len(sys.argv)!=2: raise SystemExit('usage: import_s2_mtz2_objects_phase123.py <retail Sonic 2 root>')
    ...
    # Phase 123 Act-2-specific banks. These are exact mappings rendered from the
    # same retail PLC destinations used by the level.
    specs['lava_cup']=bank(art[0x3F9*32:],(mp/'obj6C.bin').read_bytes(),'lava_cup',pals,3)
    specs['gear']=bank(art[0x378*32:],(mp/'obj70.bin').read_bytes(),'gear',pals,3)
    specs['lava_bubble']=bank(art[0x536*32:],(mp/'obj71_b.bin').read_bytes(),'lava_bubble',pals,2)
    ...
    manifest={'phase':123,'banks':specs,'tube_paths':paths,'sha256':{}}
    for q in sorted(OUT.rglob('*.png')): manifest['sha256'][str(q.relative_to(OUT))]=hashlib.sha256(q.read_bytes()).hexdigest()
    (P/'data/s1/s2test/phase123_mtz2_objects_manifest.json').write_text(json.dumps(manifest,indent=2)+'\n')
