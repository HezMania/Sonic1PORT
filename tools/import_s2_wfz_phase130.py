#!/usr/bin/env python3
"""Phase 130: retail Sonic 2 Wing Fortress boss/end integration pass.
...
def render_boss_eggman(src:Path,pals):
    """Reconstruct ObjC6 Robotnik from the three retail PLC banks at their VRAM tiles."""
    sparse=bytearray(0x600*32)
    for tile,name in [
        (0x500,"Robotnik's head.bin"),
        (0x518,"Robotnik.bin"),
        (0x564,"Robotnik's lover half.bin"),
    ]:
        raw=boss.nemesis_decompress((src/'art/nemesis'/name).read_bytes())
        sparse[tile*32:tile*32+len(raw)]=raw
    maps=(src/'mappings/sprite/objC6_a.bin').read_bytes(); n=frame_count(maps)
    d=OUT/'boss_eggman'; d.mkdir(parents=True,exist_ok=True)
    for q in d.glob('*.png'): q.unlink()
    boxes=[]
    for i in range(n):
        im=trav.render_mapping(bytes(sparse),maps,i,pals,0); im.save(d/f'{i:02d}.png'); boxes.append(im.getbbox())
    # Robotnik's small waiting platform uses ObjC5_b with the WFZ floating-platform art.
    rawp=boss.nemesis_decompress((src/'art/nemesis/Moving platform from WFZ.bin').read_bytes())
    mapsp=(src/'mappings/sprite/objC5_b.bin').read_bytes()
    sparsep=bytearray((len(rawp)//32+8)*32); sparsep[:len(rawp)]=rawp
    dp=OUT/'boss_eggman_platform'; dp.mkdir(parents=True,exist_ok=True)
    for q in dp.glob('*.png'): q.unlink()
    imp=trav.render_mapping(bytes(sparsep),mapsp,0,pals,0); imp.save(dp/'00.png')
    return {'eggman_frames':n,'eggman_bboxes':boxes,'platform_frames':1}
...
def main():
    ...
    fam={
      ...
      'boss':('art/nemesis/WFZ boss.bin','mappings/sprite/objC5_a.bin',0),
    }
    sprites={k:render_family(src,a,m,k,paln,pals) for k,(a,m,paln) in fam.items()}
    sprites['hook']=render_hook(src,pals)
    sprites['boss_eggman']=render_boss_eggman(src,pals)
    ...
    manifest={'phase':130,'level':'Wing Fortress Zone','source_revision':'retail final','runtime_palette_bridge':{'indices':[34,35,36,37],'values_hex':[pal[i*2:i*2+2].hex() for i in range(34,38)],'source':'SCZ handoff / user retail visual reference'},'start':[be16(outputs['wfz1_start.bin'],0),be16(outputs['wfz1_start.bin'],2)],
    ...
    (DATA/'phase130_wfz_manifest.json').write_text(json.dumps(manifest,indent=2)+'\n')
