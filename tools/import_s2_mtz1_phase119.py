#!/usr/bin/env python3
"""Phase 119: retail Sonic 2 Metropolis Zone Act 1 foundation.

Imports the exact retail MTZ1 terrain/layout/collision/object/ring/start streams,
...
MTZ-specific gameplay object records are preserved byte-for-byte for Phase 120;
only already-supported shared Sonic 2 objects are active in this foundation pass.
"""
...
terrain=load('terrain119',TOOLS/'import_sonic2_level.py')
bossutil=load('bossutil119',TOOLS/'import_s2_ehz_boss.py')
pres=load('pres119',TOOLS/'import_s2_ehz_presentation.py')
...
def main():
    if len(sys.argv)!=2: raise SystemExit('usage: import_s2_mtz1_phase119.py <retail Sonic 2 root>')
    ...
    layout=terrain.kosinski_decompress((src/'level/layout/MTZ_1.bin').read_bytes())
    ...
    outputs={
      'mtz1_art.bin':art,'mtz1_map16.bin':map16,'mtz1_map128.bin':map128,
      'mtz1_layout128.bin':header(128,16,fg),'mtz1_bg128.bin':header(128,16,bg),
      'mtz1_collision_primary.bin':col,'mtz1_collision_secondary.bin':col,
      'mtz1_objects.bin':(src/'level/objects/MTZ_1.bin').read_bytes(),
      'mtz1_rings.bin':(src/'level/rings/MTZ_1.bin').read_bytes(),
      'mtz1_start.bin':(src/'startpos/MTZ_1.bin').read_bytes(),
    }
    ...
    song=add_song(src)
    obj=outputs['mtz1_objects.bin']; counts=Counter(obj[i+4] for i in range(0,len(obj),6))
    active_ids={0x0D,0x26,0x36,0x41,0x79}
    active=sum(v for k,v in counts.items() if k in active_ids)
    manifest={
      'phase':119,'level':'Metropolis Zone Act 1',
      'start':[be16(outputs['mtz1_start.bin'],0),be16(outputs['mtz1_start.bin'],2)],
      'limits':{'left':0,'right':0x2280,'top':-0x100,'bottom':0x800},
      ...
      'objects':len(obj)//6,'rings':len(outputs['mtz1_rings.bin'])//6,
      'active_shared_records':active,'deferred_mtz_specific_records':len(obj)//6-active,
    ...
    (DATA/'phase119_mtz1_manifest.json').write_text(json.dumps(manifest,indent=2)+'\n')
