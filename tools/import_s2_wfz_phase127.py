#!/usr/bin/env python3
"""Phase 127: retail Sonic 2 Wing Fortress Zone foundation.
...
def render_hook(src:Path,pals):
    ...
    # SonLVL specifies art offset -128 bytes: obj80_b mapping tile indices are
    # relative to four tiles before the art block. Prepend four blank tiles so
    # the source tile-number relation remains exact in the standalone renderer.
    raw=bytes(4*32)+raw
...
def main():
    if len(sys.argv)!=2: raise SystemExit('usage: import_s2_wfz_phase127.py <retail Sonic 2 root>')
    ...
    manifest={'phase':127,'level':'Wing Fortress Zone','source_revision':'retail final','start':[be16(outputs['wfz1_start.bin'],0),be16(outputs['wfz1_start.bin'],2)],
    ...
    (DATA/'phase127_wfz_manifest.json').write_text(json.dumps(manifest,indent=2)+'\n')
